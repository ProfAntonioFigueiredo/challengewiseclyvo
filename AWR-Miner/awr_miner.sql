...
define DB_ARCH_DSP = NON-CDB
define PDB_COUNT_DSP = 0

column :DB_ARCH_1 noprint new_value DB_ARCH_DSP
variable DB_ARCH_1 varchar2(20)
column :PDB_COUNT_1 noprint new_value PDB_COUNT_DSP
variable PDB_COUNT_1 number
...
begin
	...
	if :DB_VERSION_1 >= 12.1 then
		execute immediate q'[select case when cdb = 'YES' then 'CDB' else 'NON-CDB' end from v$database]'
		into :DB_ARCH_1;

		if :DB_ARCH_1 = 'CDB' then
			execute immediate q'[select count(*) from v$pdbs where con_id > 2]'
			into :PDB_COUNT_1;
		end if;
	end if;
end;
...
set termout off
select :DB_ARCH_1 from dual;
select :PDB_COUNT_1 from dual;
...
prompt Database Type       : &DB_ARCH_DSP
prompt PDB Count           : &PDB_COUNT_DSP
...
declare
	...
	l_db_architecture	varchar2(20) := 'NON-CDB';
...
begin
	...
	if :DB_VERSION_1 >= 12.1 then
		l_sql := q'[select case when cdb = 'YES' then 'CDB' else 'NON-CDB' end from v$database]';
		execute immediate l_sql into l_db_architecture;

		if l_db_architecture = 'CDB' then
			l_sql := q'[select count(*) from v$pdbs where con_id > 2]';
			execute immediate l_sql into l_pdb_count;
		end if;
	end if;

	dbms_output.put_line(rpad('DB_ARCHITECTURE',l_pad_length)||' '||rpad('0',l_pad_length)||' '||l_db_architecture);
	dbms_output.put_line(rpad('PDB_COUNT',l_pad_length)||' '||rpad('0',l_pad_length)||' '||l_pdb_count);
	...
	if :DB_VERSION_1 <= 11.2 then
		:DB_SIZE_QUERY_1 := q'! ts_info as (
			...
			select s.snap_id,
                   ...
                   nvl(
                     nullif(round(sum(sp.tablespace_usedsize * ts.block_size)/1024/1024/1024, 2), 0),
                     round(sum(sp.tablespace_size * ts.block_size)/1024/1024/1024, 2)
                   ) as used_size_gb
...
with
	...
	,v_segstats as (
		select CASE WHEN :DB_VERSION_1 > 11.2
			...
			,round(sum(segs.bytes /1024 /1024 /1024) over(partition by segs.con_id, segs.owner, segs.segment_type, segs.segment_name), 3) as seg_size_gb
		...
		from (
			select CASE WHEN :DB_VERSION_1 > 11.2
					THEN/*+ MATERIALIZE NO_MERGE PARALLEL(&PARALELLISM_DEGREE) */stats.con_id
					ELSE stats.con_id
					END as con_id,
				stats.owner,
				stats.object_type,
			...
			where :DB_VERSION_1 >= 12.1
				and stats.owner not in (&segment_io_schema_exclusions)
				and stats.con_id = ts.con_id
				and stats.tablespace_name= ts.tablespace_name
				and ts.contents <> 'TEMPORARY'
			union all
			select CASE WHEN :DB_VERSION_1 > 11.2
					THEN/*+ MATERIALIZE NO_MERGE PARALLEL(&PARALELLISM_DEGREE) */0
					ELSE 0
					END as con_id,
				stats.owner,
				stats.object_type,
		...
		pivot (
			...
			,(
				select CASE WHEN :DB_VERSION_1 > 11.2
						THEN/*+ MATERIALIZE NO_MERGE PARALLEL(&PARALELLISM_DEGREE) */s.con_id
						ELSE s.con_id
						END as con_id,
					s.owner,
					s.segment_type,
		...
		where vss.con_id = segs.con_id(+)
			and vss.subobject_name = segs.partition_name(+)
